...
?>
 ...
 <head>
    ...
    <style>
        ...
        @media print{
            /* menghilangkan class html sehingga tampilan table saat di print tidak muncul buton inpul dll... klik tombol burger icon pilih print */
            .button, .input2, .button-update, .button-delete, .aksi{
                display:none;
                text-decoration:none;
            }
        }
    </style>
    <!-- jika script dapat dibaca jika diletakkan di head maka gunakan jquery setelah itu baru dibawahnya syntax javascript -->
 ...
 <h1>Daftar Mahasiswa</h1>
  ...
  <a href="tambah.php" > <button class="btn-hover color-1"> TAMBAH </button></a> |
  <a href="cetak.php" target="_blank"> <button class="btn-hover color-2">Cetak</button></a> |
  <a href="logout.php" > <button class="btn-hover color-3">LOGOUT</button> </a>  
  <br><br><br>
<!-- apakah datanya tampil di url atau tidak kalau idak gunakan $_POST -->
  <form action="" method="post">
    <!-- attribut input ada id sama name agar javascript dengan mudah mengambil element ini/mengetahui data yang diketikkan user sehingga ajax agak jalan melakukan pencarian -->
   <br>
  <nav class="container">
   <div><input type="text" name="keyword" size="40" autofocus placeholder="masukkan keyword pencarian..." autocomplete="off"  id="keyword" class="input2"></div>
...
<!-- tombol berfungsi untuk mencari data menggunakan php pada saat melakukan pencarian di input keyword-->
 
 <button type="submit" name="cari" class="button" id="tombol-cari">Cari!</button>
 <br><br><br>
<div ><img src="img2/loader.gif" class="loader"></div>
 </nav>
 <form>
  <br>
  ...
  <a class="aksi"  href="?halaman=<?= $halamanAktif - 1 ?>">&laquo;</a> 
  <?php endif; ?>
  ...
  <?php for( $i=1; $i <= $jumlahHalaman; $i++) : ?> 
    <!-- pindah ke halaman yang sama tapi tidak perlu menuliskan index.php?halaman... -->
        <?php if( $i == $halamanAktif) : ?>
            ...
            <a class="aksi" href="?halaman=<?= $i; ?>" style="font-weight: bold; color:red;"><?= $i; ?></a>
        
        <?php else : ?>
        <a class="aksi" href="?halaman=<?= $i; ?>"><?= $i; ?></a>
         <?php endif; ?>
  ...
  <!-- membuat tombol pagination left dan tombol ini dimunculkan ketika halaman aktifnya >1 karna kalau >1 tombolnya dipencet maka akan terjadi error karena tidak ada halaman sebelumnya-->
  <a class="aksi" style="text-decoration:none;" href="?halaman=<?= $halamanAktif + 1 ?>">&raquo;</a> 
  <?php endif; ?>
<!-- div tempat untuk menampung data hasil permintaan user yang kemudian di response oleh ajax -->
<div id="container">
<!-- membuat table -->
...
<!-- syntax javascript -->
<!-- kenapa diletakkan disini agar halamannya di load dulu ke memory-->
<!-- kalau scriptnya disimpan di head script2.js nya maka yang akan terjadi adalah sciptnya terlebih dahulu dibaca atau di load ke memory baru kemudian element" html seperti input dan button dibawahnya karena di rendernya dari atas kebawah dan dari kiri kiri kekanan olehkarena itu jika ada pemanggilan element html di scipt2 tidak akan di ketahui -->
<!-- <script src="js/jquery-3.6.4.min.js"></script>
<script src="js/script.js"></script> -->
 </body>
 </center>
